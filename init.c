...
/*   Updated: 2024/05/28 18:43:40 by spascual         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_x_ate_mutex(t_rules *rules)
{
	int i;

	i = 0;
	rules->x_ate_mutex = malloc(sizeof(pthread_mutex_t) * rules->number_of_philosophers);
	if (!rules->x_ate_mutex)
		return 1;
	while (i < rules->number_of_philosophers)
	{
		if (pthread_mutex_init(&rules->x_ate_mutex[i], NULL) != 0)
		{
			i = 0;
			while (i-- > 0)
				pthread_mutex_destroy(&rules->x_ate_mutex[i]);
			free(rules->x_ate_mutex);
			return (1);
		}
		i++;
	}
